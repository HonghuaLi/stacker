...
//--------------------------------------------------------------------------
// Constructor
Ridge_approximation::Ridge_approximation(QSurfaceMesh &m,
	...
	: M(m), k1(vertex2k1_pm), k2(vertex2k2_pm), b0(vertex2b0_pm), b3(vertex2b3_pm), 
...
OutputIterator Ridge_approximation::compute_ridges(Ridge_interrogation_type r_type, 
	OutputIterator ridge_lines_it, Ridge_order ord)
{
	tag_order = ord;
...
void Ridge_approximation::addback(Ridge_line* ridge_line, const Halfedge he,	const Ridge_type r_type)
{
	...
	double k_second = 0; // abs value of the second derivative of the curvature
...
{
	double b_p = 0., b_q = 0.; // extremalities at p and q for he: p->q
